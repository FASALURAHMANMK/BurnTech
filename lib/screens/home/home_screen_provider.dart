...
Future<List<dynamic>> getMyTickets(UserModel user) async {
    ...
    final userArts = arts.where((art) => user.artTokens?.contains(art.uid) ?? false).toList();
    final userCamps = camps.where((camp) => user.campTokens?.contains(camp.uid) ?? false).toList();
...
Future<List<EventModel>> getUpcomingEvents() async {
  ...
  Future<List<String>> getAnnouncements() async {
    ...
    if (events.isNotEmpty) {
      announcements.add("Don't miss the event: ${events.first.title ?? 'Event'}!");
    }
